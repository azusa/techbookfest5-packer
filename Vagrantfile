...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.hostname="jira.local"
  config.vm.box = "centos7.5-1804-1"
  config.vm.network :private_network, ip: "192.168.3.11"
  config.vm.network :forwarded_port, guest: 22, host: 2211
  config.vm.provision :shell, :path => "provisioning-vagrant.sh", :args => ["local"]
  ...
  config.vm.hostname = "atls-jira-local"
  ...
  #   chef.add_recipe "iptables-off"
  #   chef.add_recipe "ntpd"
